...
function WSS({ model: wss }: WssProps<string>) {
  const [uri, setUri] = useState("http://localhost:3000/chat");
  ...
  return (
    <Wrapper>
      ...
      <Group>
        <Button tabIndex={4} disabled={!connected} type="button" onClick={send}>
          Send
        </Button>
        <Button tabIndex={5} type="button" onClick={clear}>
          Clear
        </Button>
      </Group>
      <Group>
